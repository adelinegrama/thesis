...
Контроллер -- точка входа HTTP-запросов в \acs{API}. Он реализует бизнес-логику приложения и действует как мост между \acs{HTTP} и сущностями базы данных. 
...
Внедряются зависимости, где \texttt{ShopContext} дает доступ к базе данных через \acs{EF} Core, а \texttt{IMapper} (из \texttt{AutoMapper}) 
